...
- 用例池可以大于当前自动化覆盖范围
- 正式回归采用“增量回填”口径：
  - 单个 case 一旦跑成功，且结果可结构化提取，就立即回填到 `QA_TEST_PLUS_PROGRESS.md`
  - 单个 case 若未跑通，则必须写明失败摘要，至少包含 `stage/location/provider_model/raw_error/fallback`
  - 下一轮默认只重跑上一轮 `FAIL/BLOCKED` 的 case，不要求把已成功 case 从头到尾再跑一遍
  - 只有在策略、评估口径、fixture、核心 provider 配置发生变化时，才要求重新跑整套受影响用例
- `自动化状态` 用来区分阶段性落地范围：
...
- 回填策略采用“成功即落账、失败留摘要”：
  - 已成功且证据完整的 case，直接写入进度文档，后续默认不重复执行
  - 未成功的 case，保留失败摘要和定位信息，后续优先单独重跑这些失败项
  - 若某次只修复个别 provider / evaluator /策略问题，可只追加一个“失败项重跑”区块，不要求复制整套 run
- 后续新增能力时，继续沿用当前回填格式，只追加新的用例、新的 run 区块，或新的失败项重跑区块。

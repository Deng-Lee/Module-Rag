...
* `Transformer&Enrichment(post)`
  ...
  * 输出：`chunk_retrieval_text`（检索视图，默认启用）+ `chunk_metadata`（`asset_ids/anchors/tags/...`）
  * 说明：`chunk_retrieval_text` 由 `transform_post` 以模板把 `chunk_text` 与可选增强片段（标题原文/OCR/Caption/关键词等）拼接得到，**默认同时作为 dense embedding 与 sparse 索引的输入**；该阶段不做编码，只做“视图组装”，编码与索引由后续阶段完成，便于 A/B（换模板/增强字段不改 encoder）。
* `Embedding`
...
4. **检索约束**：
   ...
   * 编码/召回侧的实际输入由“编码策略/检索视图”决定：
     * 默认：以 `chunk_retrieval_text` 作为 dense/sparse 的主体输入（当未启用任何增强时，`chunk_retrieval_text == chunk_text`）；
     * 回退（facts-only）：以 `chunk_text` 作为 dense/sparse 的主体输入（仅在显式配置下启用，作为对比基线或故障兜底）。

5. **稳定性约束**：

   * 必须可通过 `canonical(chunk_text)` 生成稳定指纹；
   * `chunk_text` 不变时，`chunk_fingerprint` 必须稳定；`chunk_retrieval_text` 不变时，`content_hash` 必须稳定（用于 embedding 幂等与缓存复用）。
...
* **chunk 指纹（用于 chunk_id 稳定）**：`chunk_fingerprint = sha256(canonical(chunk_text, text_norm_profile))`
* **索引/缓存键（用于 embedding 幂等与复用）**：`content_hash = sha256(canonical(chunk_retrieval_text, text_norm_profile))`
* **embedding 输入（默认）**：`embedding_input = canonical(chunk_retrieval_text, text_norm_profile)`（建议与 `content_hash` 使用同一 profile，保证“缓存键 == 实际向量输入”）
...
2. **元数据注入（Metadata Injection）**
   ...
   * 不拼接进 `chunk_text`；是否进入检索视图由 `chunk_retrieval_text` 模板/profile 决定（默认进入，并同时作用于 dense+sparse 的索引输入）。

3. **多模态增强（Multimodal Enrichment）**

   * 资产归一化后，允许为图片资产挂载扩展产物（OCR 文本、Caption、尺寸/类型等），并通过 `asset_id` 与 chunk 关联；
   * 多模态增强结果以独立字段存储（SQLite），并可被 `transform_post` 组装进 `chunk_retrieval_text`，从而默认参与 dense+sparse 召回；同时保留 sidecar 以便回放、诊断与 Dashboard 展示。
...
* **Post-chunk（切分后）**：
  ...
  * 默认不改变 `chunk_text`（事实层），避免污染可引用原文；**embedding / sparse 索引默认使用 `chunk_retrieval_text`（检索视图）**，从而让增强“进检索”但不“进事实层”。

实现上采用同一模块的两次 hook（pre/post）完成，但对外仍视作同一阶段能力；pipeline 拓扑不得分叉。

##### 3.1.7.3 关键约束（必须遵守）

* **不得语义改写事实层**：任何 LLM 摘要/关键词/OCR/Caption 等增强默认作为独立字段（metadata / sidecar）存储，**不进入 `chunk_text`**；增强是否参与召回由 `chunk_retrieval_text` 的模板/profile 决定（默认参与并进入 dense+sparse 的索引输入）。
...
##### 3.1.7.4 增强参与召回但不污染事实层（检索视图说明）

本系统默认采用如下策略：

> 增强数据默认参与“候选发现”（dense + sparse），但不进入事实层 `chunk_text`；增强通过 `chunk_retrieval_text`（检索视图）进入索引输入。

其核心思想是：**将“事实层（可引用原文）”与“检索视图（可召回信号）”分层管理**。

###### 1. 增强信号的使用路径

增强产物（如 `chunk_summary`、`chunk_keywords`、`image_caption`、`ocr_text`）默认存储于 chunk / asset sidecar 中，并在 `transform_post` 以模板组装为 `chunk_retrieval_text`：

1. **索引输入（Dense + Sparse，默认）**：

   * `dense`: `embedding_input = canonical(chunk_retrieval_text, text_norm_profile_id)`；
   * `sparse`: `fts5_text = chunk_retrieval_text`（或其派生字段）；
   * 这样“caption/OCR 里的关键词”既能被稀疏检索命中，也能通过向量召回提升语义覆盖。

2. **过滤与偏好（可选）**：

   * 结构字段（如 `doc_id/version_id/section_path/page_range/asset_ids`）优先走索引可执行的 pre-filter；
   * sidecar 字段（如 `chunk_keywords/image_caption/ocr_text`）可作为 post-filter（Hard）或 rerank features（Soft）。

3. **返回层（事实层优先）**：

   * `library.query` 的回答与引用默认以事实层 `chunk_text` 为准；
   * `chunk_retrieval_text` 默认不回传给 client（避免把“检索视图”误当“可引用原文”），仅在 trace / dashboard 中用于诊断与对比。

###### 2. 关键收益（默认策略的工程意义）

* **增强进检索但不污染原文**：答案引用仍可严格对齐到 `chunk_text`（事实层），避免“caption 幻觉”变成引用事实。
* **召回能力可演进**：通过替换模板/profile 就能改变“哪些增强进入检索视图”，无需改 encoder / retriever，便于 A/B 与回归。
* **多模态闭环统一**：Markdown 与 PDF 最终都通过 `asset_ids[]` 关联回图片资产，OCR/Caption 既能提高命中，又能让 client 按需拉图验证。

###### 2. 为什么默认不把增强写入事实层 chunk_text（而走检索视图）

本系统把 `chunk_text` 定义为**可引用事实**：用于 citations/溯源展示与最终回答的证据对齐。因此，任何增强（摘要/关键词/OCR/Caption）都**不得写入 `chunk_text`**，而应进入 `chunk_retrieval_text`（检索视图）与 SQLite sidecar。

这样做的收益与取舍：

* **证据链更可信**：即便检索命中主要来自 caption/OCR 的关键词，返回给用户的引用仍严格指向事实层 `chunk_text`；
* **漂移可定位**：`chunk_fingerprint` 绑定事实层，`content_hash` 绑定检索视图；召回变化可区分为“事实变化”还是“增强/模板变化”；
* **策略变化可控**：增强导致 embedding/FTS5 输入变化是预期行为，但必须通过 `retrieval_template_id/enrichment_profile_id` 版本化记录并可回放；需要对比基线时可切换到 facts-only 的检索视图。

结论：**事实层稳定，检索视图可演进；增强默认进 dense+sparse，但永不污染事实层。**
...
#### 3.6.2.1 OCR/Caption 闭环要点（PDF / Markdown 对齐）

为了让“搜文字出图”在 PDF 与 Markdown 下都闭环，本系统对 OCR/Caption 的输入选择与落库方式做统一约束：

* **输入选择（如何找到要处理的图片）**：
  * Markdown：以 `asset://<asset_id>` 为主（来自 Asset Normalizer 的引用改写），可在 `chunk_text` 中直接扫描得到；并辅以 `chunk_metadata.asset_ids[]` 作为兜底。
  * PDF：事实层 `md/chunk_text` **不注入图片占位符**，因此必须依赖 `chunk_metadata.asset_ids[]`（由 `assets[].anchor -> chunk` 的挂载规则产出）来定位要 OCR/Caption 的图片集合。
* **产物落点（存哪里，怎么被检索用到）**：
  * 资产级产物（如 `ocr_text/image_caption`）以 `asset_id` 维度写入 SQLite sidecar（`asset_enrichments` 语义）。
  * chunk 级聚合产物（如 `vision_snippets[]`、`chunk_retrieval_text` 组装所需的增强片段摘要）以 `chunk_id` 维度写入 SQLite sidecar（`chunk_enrichments` 语义），用于回放/诊断/Dashboard。
  * `transform_post` 读取 sidecar 与 `chunk_metadata.asset_ids[]`，按模板组装 `chunk_retrieval_text`，并默认让增强同时进入 **dense embedding** 与 **sparse（FTS5/BM25）** 的索引输入。
* **返回语义（为什么仍是 asset_ids）**：
  * `library.query` 仍以事实层 `chunk_text` 作为引用依据，仅返回 `asset_ids[]` 供 client 按需拉取图片核验；OCR/Caption 仅用于“更容易召回到正确 chunk”与“Dashboard 可解释”。

落地改动清单（对齐目录结构与 Provider 契约，供实现对照）

* `libs/providers/enricher/*`：VisionEnricher provider 需支持从 `chunk_metadata.asset_ids[]`（PDF）与 `asset://`（Markdown）两种路径收集待处理图片；
* `ingestion/stages/transform/transform_post`：统一组装 `chunk_retrieval_text`（默认包含增强片段），并产出可回放的 `vision_snippets`/profile 记录；
* `ingestion/stages/storage/sqlite`：补齐 `asset_enrichments` / `chunk_enrichments` sidecar 的 upsert/fetch/delete；
* `ingestion/stages/storage/upsert`：在写入 chunk/embedding 时同步写入 chunk_enrichments（用于回放与 Dashboard drill-down）；
* `core/query_engine/stages/context_build`：按需 join enrichments（仅用于 trace/dashboard/调试展示，不影响事实层引用）；
* `observability/dashboard/api`：暴露 chunk 详情时可携带 enrichments（如 `/api/chunk/{chunk_id}`）。
...
flowchart TD
  ...
  E["文本增强阶段（可选，Text Proxy）<br/>- OCR：图片/扫描页 -> ocr_text<br/>- Caption：图表/流程图 -> caption_text<br/>- 写 SQLite sidecar（asset_id/chunk_id）<br/>- transform_post 组装 chunk_retrieval_text（默认进 dense+sparse）"]
...
13. **C-13 VisionEnricher 输入闭环：PDF 用 `asset_ids[]`，Markdown 扫描 `asset://`（并去重）**

目的：确保 OCR/Caption 的触发不依赖“事实层文本里出现 asset://”；实现 PDF/Markdown 两种输入路径对齐，为“搜文字出图”打通前置条件。

修改/新增文件（可见变化）：`src/libs/providers/enricher/*`、`tests/unit/test_vision_enricher_inputs.py`。

实现函数（最小集合）：

* `collect_asset_ids(chunk_text, chunk_metadata) -> asset_ids[]`
* `VisionEnricher.enrich(chunk, runtime, params) -> VisionSnippets`（内部使用 `asset_ids[]` 去重后执行）

验收标准：

* Markdown：当 `chunk_text` 含 `asset://<asset_id>` 时可收集到待处理图片；
* PDF：当 `chunk_text` 不含 `asset://` 但 `chunk_metadata.asset_ids[]` 非空时仍可触发 OCR/Caption；
* 同一 chunk 内重复引用的同一 `asset_id` 只处理一次（避免重复成本）。

测试方法：

* 单测：分别构造 Markdown/PDF 两类 chunk 输入，断言 `collect_asset_ids` 的结果正确且去重；
* 单测：FakeVisionEnricher 记录被调用的 asset_ids，断言 PDF 场景会被调用。

14. **C-14 SQLite sidecar：落库 `asset_enrichments` / `chunk_enrichments`（幂等 upsert + 可回读）**

目的：把 OCR/Caption 等增强做成“可回放、可诊断”的持久化证据链；Dashboard/Trace 可以按需 join 展示，不依赖重跑增强。

修改/新增文件（可见变化）：`src/ingestion/stages/storage/sqlite.py`、`tests/unit/test_sqlite_enrichments_sidecar.py`。

实现函数（最小集合）：

* `SqliteStore.ensure_schema()`（包含 sidecar 表 DDL）
* `SqliteStore.upsert_asset_enrichment(asset_id, provider_id, model, profile_id, ocr_text?, caption_text?, raw_json?)`
* `SqliteStore.upsert_chunk_enrichment(chunk_id, provider_id, model, profile_id, retrieval_template_id, vision_snippets_json?)`
* `SqliteStore.fetch_chunk_enrichments(chunk_ids) -> {chunk_id: enrichment}`

验收标准：

* 对同一 `(asset_id, provider_id, model, profile_id)` 的重复写入是幂等的（upsert）；
* 对同一 `(chunk_id, provider_id, profile_id)` 的重复写入是幂等的（upsert）；
* 可通过 `chunk_id` 回读到 `vision_snippets/template_id/profile_id`（用于回放/诊断/Dashboard）。

测试方法：

* 单测：临时 SQLite 中创建 schema，重复 upsert 两次，断言行数不增长；
* 单测：fetch 结果结构稳定且字段齐全（允许大字段截断）。

15. **C-15 Transform Post：执行 OCR/Caption + 写 sidecar + 组装 `chunk_retrieval_text`（默认增强进入 dense+sparse）**

目的：在 post 阶段完成“增强执行 + 视图组装 + 证据落库”的闭环：事实层不变，检索视图可演进，并保证后续编码阶段输入纯字符串（A/B 友好）。

修改/新增文件（可见变化）：`src/ingestion/stages/transform/transform_post.py`、`src/ingestion/stages/transform/retrieval_view.py`、`tests/unit/test_transform_post_vision.py`。

实现函数（最小集合）：

* `TransformPostStage.run(chunks, runtime, params) -> EnrichedChunks`
* `build_vision_snippets(asset_ids, asset_enrichments) -> vision_snippets[]`（可回放结构）
* `build_chunk_retrieval_text(chunk_text, vision_snippets, template_id) -> str`

验收标准：

* `chunk_text` 不被修改（事实层不可污染）；
* 当启用 VisionEnricher（Fake/Real）时，sidecar 中出现对应 `asset_enrichments/chunk_enrichments`；
* `chunk_retrieval_text` 默认包含增强片段（caption/OCR 摘要等），且后续 dense/sparse 使用该视图（由配置/编码策略控制）。

测试方法：

* 单测：FakeVisionEnricher 产出固定 caption/OCR，断言 `chunk_retrieval_text` 含增强片段且 `chunk_text` 不变；
* 单测：sidecar 写入被调用并可回读（mock SqliteStore 或使用临时库）。

16. **C-16 Hash/编码口径对齐：`chunk_fingerprint`（事实）vs `content_hash`（检索视图）+ 编码输入默认 `chunk_retrieval_text`**

目的：把“事实层稳定”与“检索视图可演进”落实到 hash/缓存/编码输入：事实变化与增强变化可区分；embedding cache 复用可解释。

修改/新增文件（可见变化）：`src/ingestion/stages/embedding/dense.py`、`src/ingestion/stages/embedding/sparse.py`、`src/ingestion/stages/embedding/models.py`、`tests/unit/test_hash_and_encoding_contracts.py`。

实现函数（最小集合）：

* `compute_chunk_fingerprint(chunk_text, text_norm_profile_id) -> str`
* `compute_content_hash(chunk_retrieval_text, text_norm_profile_id) -> str`
* `DenseEncoder.encode(text=chunk_retrieval_text) -> vector`
* `SparseEncoder.encode(text=chunk_retrieval_text) -> fts5_doc`

验收标准：

* 仅增强变化（caption 变化）时：`chunk_fingerprint` 不变、`content_hash` 变化；
* dense/sparse 默认使用 `chunk_retrieval_text` 作为输入（无增强时 `chunk_retrieval_text == chunk_text`）；
* embedding cache key 使用 `content_hash`（与实际编码输入一致）。

测试方法：

* 单测：构造“同 chunk_text，不同 caption”的两组输入，断言 fingerprint/content_hash 的分离行为；
* 单测：编码阶段拿到的输入字符串等于 `chunk_retrieval_text`（用 spy/fake encoder 断言）。
...
10. **D-10 多模态召回回归：caption/OCR 关键词命中也能召回 chunk（闭环验证）**

目的：固化“搜文字出图”的核心闭环：关键词只出现在 OCR/Caption（增强层）时，仍能通过 dense/sparse 任一路召回对应 chunk，并返回正确的 `asset_ids[]` 供 client 按需拉取验证。

修改/新增文件（可见变化）：`tests/integration/test_multimodal_caption_recall.py`、（必要时）`tests/fixtures/docs/*`（含图片的 pdf/md fixture）。

实现函数（最小集合）：

* `run_ingest_fixture(..., enable_vision=true) -> {doc_id, version_id}`
* `run_query_fixture(query, strategy_config_id) -> ResponseIR`

验收标准：

* 构造用例：事实层 `chunk_text` 不包含关键词，仅 OCR/Caption 含关键词；
* query 该关键词：Top-K sources 包含目标 `chunk_id`，且 `asset_ids[]` 非空；
* trace 中包含 `stage.transform_post` 的 vision enrichment 记录（provider/model/profile 可见）。

测试方法：

* 集成：先 ingest fixture（FakeVisionEnricher 或 REAL），再 query 关键词；断言命中与 `asset_ids[]`；
*（可选）REAL：在本机环境启用真实 Vision provider，允许结果有轻微波动，但必须命中同一文档范围。

11. **D-11 事实层引用不被污染：Response/Citations 仍指向 `chunk_text`，检索视图仅用于诊断**

目的：把“增强默认进 dense+sparse”与“事实层可引用”同时落地：即便主要命中来自 caption/OCR，返回给用户的引用仍严格对齐事实层 `chunk_text`，避免把增强当作引用证据。

修改/新增文件（可见变化）：`src/core/query_engine/stages/context_build.py`、`src/core/query_engine/stages/format_response.py`、`tests/unit/test_fact_vs_retrieval_view_response.py`。

实现函数（最小集合）：

* `ContextBuildStage.run(...) -> ContextBundle`（回读事实层 chunk_text + 结构化 metadata）
* `format_response(bundle, ...) -> ResponseIR`（默认不回传 `chunk_retrieval_text`）

验收标准：

* ResponseIR.sources 中展示/引用的正文片段来自事实层 `chunk_text`；
* `chunk_retrieval_text` 不出现在 tool 的默认返回里（只在 trace/dashboard 可见）；
* 当切换 facts-only 检索视图时（显式配置），输出仍一致可回归。

测试方法：

* 单测：构造一个 `chunk_retrieval_text` 与 `chunk_text` 明显不同的样本，断言返回引用片段取自 `chunk_text`；
* 单测：trace 里可以看到 retrieval view（若已记录），但 response 不回传该字段。
...
9. **G-9 Chunk 详情补齐多模态侧证据：API 返回 enrichments（sidecar join），前端可展示**

目的：让 Dashboard 的“数据浏览器/Chunk 详情”能直接看到多模态增强（caption/OCR/vision_snippets）以及其 profile/template 信息，支持 drill-down 与回归对比；同时保持前端不硬编码 provider 细节（JSONTree/KVTable 兜底展示）。

修改/新增文件（可见变化）：`src/observability/dashboard/api.py`、`src/observability/readers/sqlite_reader.py`、`web/src/routes/Browser.tsx`、`tests/e2e/test_dashboard_smoke.py`。

实现函数（最小集合）：

* `GET /api/chunk/{chunk_id}`：返回 `{chunk, assets, enrichments?}`（enrichments 可截断）
* `SqliteReader.get_chunk_detail(chunk_id) -> ChunkDetail`（join chunk_enrichments/asset_enrichments）

验收标准：

* `/api/chunk/{chunk_id}` 响应中包含 `enrichments`（如 `vision_snippets/template_id/profile_id`，以及 `asset_enrichments` 摘要字段）；
* Browser 页面能以表格/KV/JSONTree 展示 enrichments（不要求复杂 UI，保证可读与可定位即可）；
* 大字段（OCR 全文/长 caption）必须可截断或分页，避免页面/stdio payload 膨胀。

测试方法：

* 集成：TestClient 请求 `/api/chunk/{id}`，断言 enrichments 字段存在且 shape 稳定；
* e2e：在 dashboard smoke 中增加该接口的 JSON shape 校验（不做 DOM 断言）。
...
| C-13 | VisionEnricher 输入闭环（PDF asset_ids / MD asset://） | 完成 | 2026-03-05 | `collect_asset_ids`、PDF/MD 去重收集 |
| C-14 | SQLite sidecar：asset/chunk enrichments | 完成 | 2026-03-05 | `upsert_asset_enrichment`、`upsert_chunk_enrichment`、`fetch_chunk_enrichments` |
| C-15 | Transform Post：OCR/Caption + 视图组装 | 完成 | 2026-03-05 | `TransformPostStage.run`（写 sidecar + retrieval view） |
| C-16 | Hash/编码口径对齐（fingerprint vs content_hash） | 完成 | 2026-03-05 | `compute_chunk_fingerprint`/`compute_content_hash` + 编码输入对齐 |
...
| D-10 | 多模态召回回归（caption/OCR 命中） | 完成 | 2026-03-05 | `test_multimodal_caption_recall.py`（命中+asset_ids） |
| D-11 | 事实层引用不污染（response 仅 chunk_text） | 完成 | 2026-03-05 | `test_multimodal_caption_recall.py`（retrieval view 不泄露） |
...
| F-7 | golden trace 回归 | 完成 | 2026-02-28 | `tests/golden/trace_query.json` + `test_trace_golden.py` 快照断言 |
| F-8 | 稳定性契约回归（ID/规范化/切分） | 完成 | 2026-02-28 | canonical/chunk_id/asset_id/ref_id/md_norm 稳定性测试 |
...
| G-9 | Chunk 详情补齐多模态 enrichments 展示 | 完成 | 2026-03-05 | `/api/chunk/{id}` join sidecar + Browser 展示 |

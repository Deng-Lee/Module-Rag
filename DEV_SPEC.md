...
两段式切分的核心目标是：**把“结构边界决定权”与“长度控制/策略试验”解耦**，从而降低 chunk 漂移的影响半径，并提升可诊断性。

* **降低漂移半径（Localize Drift）**：

  * 结构切分（Sectioner）变化通常来自解析/标题识别变化，一旦变化会影响一整段内容；
  * 长度切分（Chunker）变化通常来自参数/算法实验；
  * 两者拆开后，长度策略的调整只在 **section 内局部生效**，避免“全篇边界整体偏移”。

* **提高可诊断性（Diagnose Drift）**：

  * 若 `section_count/section_hash` 变化 → 结构漂移；
  * 若 `section_count` 不变但 `chunk_count/chunk_hash` 变化 → 长度漂移；
  * 这使得回放与回归定位不依赖“读日志猜”。

* **增强可插拔性（Plug-in Friendly）**：

  * “按结构切分”的策略实现为 Sectioner（如 headings/AST/rule_based/whole_doc）；
  * “按长度/语义切分”的策略实现为 Chunker（如 RCTS/token/semantic/sentence_window）；
  * 固定 pipeline 拓扑不变（Rule2），仅替换 provider 即可试验。
...
* **内容驱动变化（Content-driven Change）**：文档正文发生修改、增加或删除。这种变化属于事实层演进，理应触发对应 section 与 chunk 的重建与重新向量化，属于“合理重建”。
* **策略驱动变化（Strategy-driven Drift）**：在文档内容未发生实质变化的情况下，由解析规则、标题识别、Sectioner/Chunker 参数或算法变更引起的 chunk 边界或数量变化。这类变化不源于事实层，而源于系统策略层，必须被显式记录与诊断。

本节所讨论的“chunk 漂移”重点关注**策略驱动变化**，其风险在于：结构或参数微调可能导致 embedding 与检索结果整体偏移，却难以归因。

在本系统中将漂移拆为两类：

* **结构漂移（Structure Drift）**：Sectioner 的边界变化（标题识别、层级变动、短段合并策略变化等）导致 section 发生增删或范围变化。
* **长度漂移（Length Drift）**：在 section 不变的前提下，Chunker 的参数/算法变化（chunk_size/overlap/separators/length_function 等）导致 chunk 边界变化。
...
4. **检索约束**：
   ...
   * 是 Dense embedding 的主体输入；
   * 是 FTS5/BM25 建索引的主体文本；
   * 增强字段（summary / keywords / caption 等）默认不混入其中。
...
SQLite Join (结构补全)
   - 根据 chunk_id 获取:
       ...
       asset_ids
       chunk_summary / keywords (sidecar)
   │
   ▼
Optional Rerank (Cross-Encoder / LLM Rerank)
   - 输入: query + chunk_text (+ sidecar features)
   - 输出: reranked_top_k
   │
   ▼
Answer Generation (LLM)
   - 拼接最终上下文
   - 可附带图片 caption / OCR 信息
   │
   ▼
Final Response
```
...
修改/新增文件（可见变化）：`src/libs/providers/loader/pdf_loader.py`、`src/libs/interfaces/loader/loader.py`（如需补齐接口细节）、`tests/fixtures/pdf/*`（最小 PDF 样本）。
...
| 任务编号 | 任务名称 | 状态 | 完成日期 | 备注（关键实现） |
|---|---|---|---|---|
| B-1 | ProviderRegistry（注册与实例化） | 未完成 |  | `register/get/create/has`、缺失 provider 异常 |
| B-2 | 工厂层 Factories（装配 graph） | 未完成 |  | `make_*` 系列装配、可选模块 `noop` |
